...
import Tool from "@/components/FileHashChecker";
...
export const metadata: Metadata = {
  title: "File Hash Checker — Free Online Tool",
      ...
      <main className="min-h-screen px-4 py-12">
        <div className="mx-auto max-w-4xl">
          <div className="mb-10 text-center">
            ...
            <h1 className="mb-3 text-4xl font-bold text-white md:text-5xl">File Hash Checker</h1>
            <p className="mb-2 text-lg text-text-muted">Calculate the SHA hash of any file and compare it with a known one to verify its integrity.</p>
          ...
          <div className="mt-8 grid grid-cols-1 gap-4 sm:grid-cols-3">
            {[
            { icon: "📁", title: "Any file", desc: "SHA-1/256/384/512 computed straight from the file with Web Crypto." },
            { icon: "✅", title: "Known-hash compare", desc: "Paste a published hash and confirm the match instantly." },
            { icon: "🔒", title: "100% private", desc: "Files are hashed locally and never uploaded." },
            ].map((item) => (
          ...
          <div className="mt-8 rounded-xl border border-border/20 bg-white/3 p-6">
            ...
            <p className="mb-4 text-sm text-text-muted">Add File Hash Checker to any page with a simple iframe, or link to it with attribution.</p>
            <div className="mb-3 rounded-lg bg-black/40 p-3">
              <p className="mb-1 text-xs text-text-muted/60">Iframe (plug & play):</p>
              <code className="text-xs text-green-400 break-all">{`<iframe src="${EMBED_URL}" width="100%" height="700" style="border:none;border-radius:12px;" title="File Hash Checker — miguelacm.es" loading="lazy"></iframe>`}</code>
            </div>
            <div className="rounded-lg bg-black/40 p-3">
              <p className="mb-1 text-xs text-text-muted/60">Link with attribution (recommended for backlink):</p>
              <code className="text-xs text-green-400 break-all">{`<a href="${SITE_URL}" target="_blank" rel="noopener">File Hash Checker — free tool by MACM</a>`}</code>
            </div>
          </div>
        </div>
      </main>
